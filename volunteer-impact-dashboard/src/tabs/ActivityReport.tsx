    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-4xl mx-auto p-4 lg:p-8 space-y-8">
        {activeTab === 'Contributions' ? (
          ...
          <div className="space-y-8">
            <div className="bento-card p-8 bg-gradient-to-br from-primary to-secondary text-white relative overflow-hidden">
               <div className="relative z-10 space-y-6">
                 <div className="flex items-center gap-3">
                   ...
                   <div>
                     <h1 className="text-3xl font-display font-black">Hustle Board</h1>
                     <p className="text-white/60 font-medium italic">Top shares this push</p>
                   </div>
                 </div>

                 <div className="space-y-3">
                    <div className="flex justify-between items-end">
                       <span className="text-xs font-black uppercase tracking-widest text-white/70">Current Push Progress</span>
                       <span className="text-xl font-display font-black">1,280 / 2,000 <span className="text-white/60 text-sm font-medium">shares</span></span>
                    </div>
                    <div className="h-4 bg-white/20 rounded-full overflow-hidden backdrop-blur-sm border border-white/10">
                       <div className="h-full bg-brand-yellow shadow-[0_0_15px_rgba(254,197,46,0.5)]" style={{ width: '64%' }} />
                    </div>
                 </div>
               </div>
            ...
            <div className="flex bg-surface p-1 rounded-xl border border-border w-full">
              <button 
                onClick={() => setHustleView('Individual')}
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                  hustleView === 'Individual' ? 'bg-white text-primary shadow-sm' : 'text-text-secondary hover:text-text'
                }`}
              >
                Individual
              </button>
              <button 
                onClick={() => setHustleView('Team')}
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                  hustleView === 'Team' ? 'bg-white text-primary shadow-sm' : 'text-text-secondary hover:text-text'
                }`}
              >
                Team
              </button>
            ...
            <div className="bento-card overflow-hidden">
              <div className="divide-y divide-border">
                {(hustleView === 'Individual' ? individualLeaders : teamLeaders).map((leader) => (
                  <div 
                    key={leader.name} 
                    className={`px-6 py-5 flex items-center justify-between group hover:bg-surface/50 transition-all ${
                      leader.name === 'Kirk Harbaugh' ? 'bg-primary/5' : ''
                    }`}
                  >
                    <div className="flex items-center gap-6">
                      <div className={`w-8 text-xl font-display font-black ${
                        leader.rank === 1 ? 'text-brand-yellow' : 
                        leader.rank === 2 ? 'text-text-secondary/50' :
                        leader.rank === 3 ? 'text-brand-yellow/60' : 'text-text-secondary/30'
                      }`}>
                        #{leader.rank}
                      </div>
                      <div className="flex items-center gap-4">
                        {hustleView === 'Individual' && (
                          <div className="w-10 h-10 rounded-full bg-surface border border-border flex items-center justify-center font-display font-black text-xs text-text-secondary">
                            {leader.name.split(' ').map(n => n[0]).join('')}
                          </div>
                        )}
                        <div>
                          <p className={`text-base font-display font-bold ${leader.name === 'Kirk Harbaugh' ? 'text-primary' : 'text-text'}`}>
                            {leader.name}
                            {leader.name === 'Kirk Harbaugh' && <span className="ml-2 font-sans text-[10px] font-black uppercase tracking-widest opacity-60">(You)</span>}
                          </p>
                          <p className="text-[10px] text-text-secondary font-black uppercase tracking-widest opacity-60">
                            {hustleView === 'Individual' ? (leader as any).team : 'Active Team'}
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-2xl font-display font-black text-text">{leader.shares}</p>
                        <p className="text-[9px] text-text-secondary font-black uppercase tracking-widest">Push Shares</p>
                      </div>
                      {leader.rank === 1 && <Trophy size={20} className="text-brand-yellow" />}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bento-card p-6 flex flex-col gap-4 border-success/20 bg-success/5">
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 bg-success/10 rounded-xl flex items-center justify-center text-success">
                     <TrendingUp size={20} />
                   </div>
                   <h4 className="text-sm font-black uppercase tracking-widest">Push Share Leader</h4>
                </div>
                <p className="text-xs text-text-secondary font-medium leading-relaxed">
                  Elena Rodriguez has shared 12 templates during this push. 14 other Yellow Jackets are hot on her trail.
                </p>
              </div>
              <div className="bento-card p-6 flex flex-col gap-4 border-primary/20 bg-primary/5">
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center text-primary">
                     <Target size={20} />
                   </div>
                   <h4 className="text-sm font-black uppercase tracking-widest">Your Achievement</h4>
                </div>
                <p className="text-xs text-text-secondary font-medium leading-relaxed">
                  You're 6 shares away from reaching your push personal goal of 20 shares. Double points active for Football Kickoff shares.
                </p>
              </div>
            </div>
          </div>
