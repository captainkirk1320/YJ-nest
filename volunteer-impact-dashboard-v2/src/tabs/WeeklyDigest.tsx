...
import { VOLUNTEERS, GOALS } from '../constants';
...
export default function WeeklyDigest({ onBack, onTakeAction }: WeeklyDigestProps) {
  ...
  // v2: org goal moved here from Home
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-20 px-4 pt-4">
      ...
      {/* v2: 2026 Campaign Progress — relocated from Home */}
      {orgGoal && (
        <section className="bento-card p-6 space-y-3">
          <div className="flex items-center gap-2 border-b border-border pb-4">
            <Trophy size={20} className="text-primary" />
            <h2 className="text-sm font-bold uppercase tracking-[0.08em] text-text-secondary">2026 Campaign Progress</h2>
          </div>
          <div className="flex items-end justify-between">
            <div>
              <p className="text-2xl font-display font-bold text-text">${orgGoal.current.toLocaleString()}</p>
              <p className="text-xs text-text-secondary">of ${orgGoal.target.toLocaleString()}</p>
            </div>
            <p className="text-3xl font-display font-bold text-primary">{Math.round((orgGoal.current / orgGoal.target) * 100)}%</p>
          </div>
          <div className="h-2 bg-surface rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full" style={{ width: `${(orgGoal.current / orgGoal.target) * 100}%` }} />
          </div>
          {orgGoal.description && <p className="text-[10px] text-text-secondary italic">{orgGoal.description}</p>}
        </section>
      )}
